...
def main():
    ...
    history_step3 = train_step3_hdc(model, train_loader, test_loader, args, device)
    ...
    # 从 step3 history 里提取最佳指标
    if history_step3 is not None and isinstance(history_step3, dict):
        if "hdc_step3_test_acc" in history_step3 and history_step3["hdc_step3_test_acc"]:
            results["best_test_acc_routing"] = max(history_step3["hdc_step3_test_acc"])
        if "hdc_step3_keep_rates" in history_step3 and history_step3["hdc_step3_keep_rates"]:
            results["final_keep_rates"] = history_step3["hdc_step3_keep_rates"][-1]
